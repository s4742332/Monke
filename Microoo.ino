//Ultrasonic Set-up
  //float duration[3], distance[3]; //duration and distance
  int trig[3] = {5, 6, 7}; // L, F, R \ pins
  int echo[3] = {16, 14, 15};
  float lights[3] = {17, 18, 19}; //Pins for Wall Indicator lights, L, F, R, need to test if pinmode ouput works

//Motor Set-Up
  int LeftINA = 2; // Controls the direction of spin for the left motor
  int LeftINB = 3;
  int LeftSpeed = 4; //ENA Pin - Controls the speed for the right motor, must be on a PWM(~)  pin

  int RightINC = 5; //Controls the directino of spin for the right motor
  int RightIND = 6;
  int RightSpeed = 7; //ENB Pin - Controls the speed for the right motor, must be on a PWM(~) pin
  
//Global variables
  const int Walld = 4;//cm
  const int cell_wall = 16.8; //cm
  int bot_orientation=0; //F, R, B, L 
  int bot_position[2]={0,0}; // co-ordinate position of the bot (x, y)
...
float ultrasonic(int side) {
  // takes in an integer 0, 1, 2 == LEFT, FRONT, RIGHT
...
bool walls() {
  //Checks the surrounding walls in a cell
  bool iswalls[4];
...
void forward() {
  ...
  if (dist_fwall_1 >= (cell_wall-2)){ //if we know that the wall is further away than the closest a wall can be we know we can move forward
    while (dist_fwall_1 - ultrasonic(1) >= 16.75) {
...
void rotate(int direction) {
  // rotates 90 degrees either left or right 
  analogWrite(LeftSpeed, 100);  //Controlling speed (0  = off and 255 = max speed):    
  analogWrite(RightSpeed, 100); 
